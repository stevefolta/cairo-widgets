...
WaylandWindow::Style WaylandWindow::style;


struct SHMBackend : public WaylandWindow::Backend {
...
WaylandWindow::WaylandWindow(double initial_width, double initial_height)
	: CompoundWidget(&cairo_gui, { 0, 0, initial_width, initial_height }), cairo_gui(this)
{
	...
	static const struct wl_surface_listener surface_listener = {
		.enter = [](void* data, wl_surface* wayland_surface, wl_output* output) {
			((WaylandWindow*) data)->surface_enter(output);
			},
		.leave = [](void* data, wl_surface* wayland_surface, wl_output* output) {
			((WaylandWindow*) data)->surface_leave(output);
			}
		};
	...
	if (wayland_display.xdg_wm_base) {
		...
		static const struct xdg_toplevel_listener toplevel_listener = {
			.configure = [](void* data, struct xdg_toplevel* toplevel, int32_t width, int32_t height, struct wl_array* states) {
				((WaylandWindow*) data)->resize(width, height, states);
				},
			.close = [](void* data, struct xdg_toplevel* toplevel) {
				((WaylandWindow*) data)->close_requested();
				},
			};
	...
	if (wayland_display.xdg_decoration_manager_v1) {
		...
		static const struct zxdg_toplevel_decoration_v1_listener decoration_listener = {
			.configure = [](void *data, struct zxdg_toplevel_decoration_v1* decoration, uint32_t mode) {
				((WaylandWindow*) data)->server_side_decorations =
...
void WaylandWindow::mouse_pressed(double x, double y, int button, uint32_t serial)
{
	if (!server_side_decorations) {
		bool is_in_frame =
			x < style.frame_left ||
			x >= style.frame_left + rect.width ||
			y < style.frame_top ||
			y >= style.frame_top + rect.height;
		if (is_in_frame) {
...
void WaylandWindow::mouse_released(double x, double y, int button)
{
	if (!server_side_decorations) {
		bool is_in_frame =
			x < style.frame_left ||
...
void WaylandWindow::mouse_moved(double x, double y)
{
	...
	CompoundWidget::mouse_moved(x, y);
	redraw();

	int new_cursor = preferred_cursor(x, y);
...
void WaylandWindow::key_pressed(int c)
{
	if (focused_widget)
		focused_widget->key_pressed(c);
}

void WaylandWindow::special_key_pressed(SpecialKey key)
{
	if (focused_widget)
...
void WaylandWindow::entered(double x, double y)
{
	last_cursor = -1;
	mouse_moved(x, y);
}
...
void WaylandWindow::set_title(const std::string& new_title)
{
...
cairo_t* WaylandWindow::get_cairo()
{
	if (cairo)
		return cairo;

	auto total_width = rect.width;
	auto total_height = rect.height;
	if (!server_side_decorations) {
...
void WaylandWindow::resize(int32_t new_width, int32_t new_height, struct wl_array* states)
{
	...
	if (contents_width != rect.width || contents_height != rect.height) {
...
void WaylandWindow::draw_frame()
{
	auto cairo = cairo_gui.cairo();

	auto frame_width = rect.width + style.frame_left + style.frame_right;
	auto frame_height = rect.height + style.frame_top + style.frame_bottom;

	// Clear the corners to alpha.
	cairo_save(cairo);
	cairo_set_operator(cairo, CAIRO_OPERATOR_SOURCE);
	auto frame_right_x = style.frame_left + rect.width;
	auto frame_bottom_y = style.frame_top + rect.height;
	use_color({ 0, 1, 1, 0.0 });
	use_rect({ 0, 0, style.frame_left, style.frame_top });
	cairo_fill(cairo);
	use_rect({ frame_right_x, 0, style.frame_right, style.frame_top });
	cairo_fill(cairo);
	use_rect({ 0, frame_bottom_y, style.frame_left, style.frame_bottom });
	cairo_fill(cairo);
	use_rect({ frame_right_x, frame_bottom_y, style.frame_right, style.frame_bottom });
	cairo_fill(cairo);
	cairo_restore(cairo);

	// Draw the frame.
	rounded_rect({ 0, 0, frame_width, frame_height }, style.frame_left);
	// Punch a hole for the contents (no need to touch those pixels yet).
	cairo_move_to(cairo, style.frame_left, style.frame_top);
	cairo_rel_line_to(cairo, 0, rect.height);
	cairo_rel_line_to(cairo, rect.width, 0);
	cairo_rel_line_to(cairo, 0, -rect.height);
	cairo_close_path(cairo);
	use_color(style.frame_color);
	cairo_fill(cairo);
	...
	use_color(style.title_color);
	cairo_move_to(cairo, style.frame_left + (rect.width - text_extents.width) / 2, (style.frame_top + real_ascent) / 2);
...
void WaylandWindow::frame_mouse_pressed(double x, double y, uint32_t serial)
{
	uint32_t resize_edges = 0;
	double frame_right_x = style.frame_left + rect.width;
	double frame_bottom_y = style.frame_top + rect.height;
...
void WaylandWindow::frame_mouse_moved(double x, double y)
{
	if (tracking_frame_button) {
...
void WaylandWindow::frame_mouse_released(double x, double y)
{
...
void WaylandWindow::layout_frame_buttons()
{
	auto frame_width = rect.width + style.frame_left + style.frame_right;
...
void WaylandWindow::FrameButton::draw(cairo_t* cairo)
{
	const auto& style = WaylandWindow::style;
